...
- `CompletionJudge`：`final_answer` 独立判定（rubric / llm-as-judge / 无错误+无未完成 TODO）。**模型不能自我批准完成**。判定前串联两个纯代码硬校验（无 TOOL 轨迹自动豁免）：
  ...
  - `DataConsistencyVerifier`（跨领域通用）：① 任务要求"计算"（中英文任务词）时须有正向计算式（`(A-B)/B` 等；反推形 `X/(1±p)` 不算）或「引用××已公布值（非自行计算）」声明；② 答复中显著数值（带单位且基础单位 ≥1000、带万/亿倍率任意值、五位以上裸数字）须工具支撑（万/亿双向换算+1% 容差）/出现在计算式/带推导·近似标记（近似词须紧邻数值，「预约」类复合词不放行）；③ 多口径并存（挂牌/成交/零售/批发/均价/中位数等跨领域口径）须说明口径差异，同比/环比视为修饰语不算口径；④ 同期同口径同指标差异 >20% 须归因并选主口径（仅警告，喂给 llm-as-judge 评审）。
